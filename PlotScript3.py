...
# Separate data for years before and after 2000
df_before_2000 = annual_sum_prcp_snow[annual_sum_prcp_snow['YEAR'] < 2000]
df_after_2000 = annual_sum_prcp_snow[annual_sum_prcp_snow['YEAR'] >= 2000]


# Calculate the overall average annual sum for each station for each period
overall_avg_before_2000 = df_before_2000.groupby('STATION')['PRCP+SNOW'].mean().reset_index()
overall_avg_after_2000 = df_after_2000.groupby('STATION')['PRCP+SNOW'].mean().reset_index()

# Merge the two datasets to get the difference and latitude/longitude information
heatmap_data = pd.merge(
    overall_avg_before_2000,
    overall_avg_after_2000,
    on='STATION', suffixes=('_before_2000', '_after_2000')
)

# Create a new column for the difference
heatmap_data['Difference'] = heatmap_data['PRCP+SNOW_after_2000'] - heatmap_data['PRCP+SNOW_before_2000']
...
# Plot a scatter plot with colors corresponding to the difference in averages
plt.scatter(x, y, c=heatmap_data['Difference'], cmap='coolwarm_r', s=50, marker='o', alpha=0.8, vmin=-20, vmax=20)

# Add colorbar
cbar = plt.colorbar()
cbar.set_label('Difference in Avg. Annual Rain+Snowfall')

# Add labels and title
plt.title('Heatmap of Difference in Avg. Annual \nRain+Snowfall (1970-2000 and 2000-Present)')
plt.savefig('C:/Users/bryce/Downloads/MIS581_FInal/Difference_Avg_Annual_Sum.png', dpi=100)
